    ...
    return (
      <div className="max-w-6xl mx-auto px-6 py-16 pt-[150px]">
        <header className="mb-10 text-center">
          ...
          <p className="mt-3 text-gray-800/80 max-w-2xl mx-auto">{t('testimonials.intro')}</p>
        </header>
        <div className="flex items-center justify-center h-64">
          <div className="text-gray-800/60">Loading testimonials...</div>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <aside className="w-full md:w-96 mt-4 md:mt-0">
            <div className="bg-white/6 p-4 rounded-xl shadow-soft">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="p-2 bg-blue-600/80 rounded-md text-gray-800"
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-800/60">{current?.company} • {current?.year}</div>
                  </div>
                </div>
                <div className="text-right">
                  {current?.metrics && <div className="text-sm font-semibold text-green-300">{current.metrics}</div>}
                  <div className="mt-2 flex items-center justify-end gap-2">
                    <button onClick={prev} className="p-2 rounded-md bg-white/8 text-gray-800" aria-label="Previous">
                      <ChevronLeft size={18} />
                    </button>
                    <button onClick={next} className="p-2 rounded-md bg-white/8 text-gray-800" aria-label="Next">
              ...
              <blockquote className="mt-4 text-gray-800/90 text-lg">"{current?.quote}"</blockquote>

              <div className="mt-4">
                <label className="text-xs text-gray-800/60">{t('testimonials.filter.industry')}</label>
                <select
                  value={filter}
                  onChange={(e) => setFilter(e.target.value)}
                  className="w-full mt-2 rounded-md bg-black/20 text-gray-800 p-2"
              ...
              <div className="mt-4">
                <a href="#projects" className="inline-block bg-blue-500 text-gray-800 px-4 py-2 rounded-md">{t('testimonials.case.view')}</a>
              </div>
            </div>

            <div className="mt-6 bg-white/4 p-4 rounded-xl">
              <h3 className="text-sm font-medium">{t('testimonials.longform.title')}</h3>
              <ul className="mt-3 space-y-3 text-gray-800/80 text-sm">
            ...
            <div key={it.id} className="bg-white/5 p-4 rounded-xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-800/60">{it.title} — {it.company}</div>
                </div>
                <div className="ml-auto text-sm text-green-300">{it.metrics}</div>
              </div>
              <p className="mt-3 text-gray-800/80">{it.quote}</p>
